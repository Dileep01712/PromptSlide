...
const Themes: React.FC<ThemesProps> = ({ onThemeSelect }) => {
    // Manage the selected theme state (default is 1)
    const [selectedTheme, setSelectedTheme] = useState<number>(1);
    const themes = [1, 2, 3, 4, 5, 6]; // IDs for themes
    ...
    return (
        <div>
            ...
            <div className="w-full grid grid-cols-3 md:gap-4 gap-2">
                {themes.map((id) => (
                    <div key={id} className='flex flex-wrap box-border'>
                        <label className='relative cursor-pointer w-full rounded-md'>
                            ...
                            <img src={`./assets/Landing/features${id}.webp`} alt={`Theme ${id}`} className='dark:border-gray-300 border-black border-2 w-full rounded peer-checked:border-blue-500 md:h-32' />
